...
from ralph.agent_runner import collect_result, collect_result_from_file, is_api_error, is_auth_error, run_agent_async
from ralph.config import RalphConfig
from ralph.dashboard import Dashboard, NullDashboard
from ralph.decomposer import needs_decompose, run_decompose
from ralph.notifications import (
    notify_all_done,
...
from ralph.orchestrator import plan_batches
from ralph.reporter import IterationReport, Reporter, fmt_duration, generate_summary
from ralph.state_store import StateStore
from ralph.task_manager import Task, TaskManager
from ralph.tmux_runner import TmuxAgent, kill_all_ralph_sessions, launch_agent, tmux_available
...
def _log_event(log_dir: Path, event: str, **kwargs) -> None:
    """Append a structured JSON event to ralph_events.jsonl."""
    ...
    events_file = log_dir / "ralph_events.jsonl"
...
def _show_startup_banner() -> None:
    ...
    os.environ["RALPH_GIT_SHA"] = f"{sha}{dirty}"
    version_line = f"RALPH v{version}  @  {sha}{dirty}"
    banner = (
        "\n"
        f"{B}{YL}{'=' * 72}{R}\n"
        f"{B}{RD}{BGB}{WH}                                                                        {R}\n"
    ...
    sys.stderr.flush()
    log.info("ralph start: v%s @ %s%s", version, sha, dirty)
    time.sleep(5)
...
def _delete_interactive(choice: str, plans: list[Path], prds: list[Path]) -> None:
    ...
    Asks for confirmation. Also offers to remove related ralph workspace.
    """
    rest = choice[1:].strip()
    ...
    for kind, p in targets:
        try:
            ...
            if kind == "plan":
                ...
                ws = Path(".ralph_workspaces") / slug_hint
                if ws.exists() or any(Path(".ralph_workspaces").glob(f"{slug_hint}*")):
                    _ansi(f"  {D}Workspace .ralph_workspaces/{slug_hint}* остался — удали вручную если нужно{R}")
...
# ── Dashboard ────────────────────────────────────────────────
# Rich Dashboard imported from ralph/dashboard.py
...
def run_plan(
    plan_file: str, config: RalphConfig, agent_name: str, *, resume: bool = False, state_store: StateStore | None = None
) -> Path | None:
    """Execute one plan file. Returns reporter JSON path or None."""
    import os

    from ralph.worktree_runner import (
    ...
    # Plan-level workspace изолирует всю работу в .ralph_workspaces/{slug},
    ...
    try:
        while True:
            ...
            if not ready:
                ...
                dashboard.update()
                # Log heartbeat every 10 idle ticks so ralph.log doesn't go silent
                if idle_ticks % 10 == 0:
            ...
            if config.MAX_PARALLEL > 1:
                ...
                if config.ORCHESTRATOR == "llm" and len(batch) > 1:
                    from ralph.orchestrator import detect_module_overlap, run_interface_agreement
...
def _maybe_merge_workspace(workspace, tm, config) -> None:
    ...
    Режимы (env RALPH_AUTO_MERGE):
      ...
      - 'no' — skip
    """
    mode = (os.environ.get("RALPH_AUTO_MERGE") or "ask").lower()
    ...
    if mode == "no":
        _ansi(f"{D}RALPH_AUTO_MERGE=no — merge пропущен{R}")
        return
    if mode == "yes":
...
def _run_claude_merge(workspace, tm) -> None:
    ...
    mr_target = os.environ.get("RALPH_MR_TARGET", "upstream main branch")
    prompt = (
...
Environment variables:
  RALPH_MAX_ITERATIONS=N   Max work iterations per plan (0=unlimited)
  RALPH_MAX_PARALLEL=N     Max concurrent agents (1=sequential, 2-3=parallel)
  RALPH_USE_TMUX=1/0       Use tmux for parallel execution (default: 1)
  RALPH_MODEL=MODEL        Model to use (default: claude-opus-4-6[1m])
  RALPH_LOG_DIR=DIR        Directory for agent logs (default: ralph_logs)
  RALPH_AGENT=NAME         Agent name for reports
  RALPH_STATE_FILE=FILE    State file for crash recovery (default: .ralph_state.json)
  RALPH_BUDGET_USD=N       Cost budget in USD (0=unlimited, default=0)
  RALPH_MAX_TASK_RETRIES=N Max retries before skipping a task (default=5)
  RALPH_HEADLESS=1/0       Headless/CI mode (default: auto-detect from TTY)
  RALPH_TIMEOUT=N          Max wall time in seconds (0=unlimited)
"""


def resolve_agent_name(config: RalphConfig) -> str:
...
def main(argv: list[str] | None = None) -> int:
    ...
    _log_path = Path.cwd() / "ralph.log"
    os.environ["RALPH_LOG_PATH"] = str(_log_path)
    logging.basicConfig(
        level=logging.INFO,
    ...
    if "--prd-wizard" in args:
        from ralph.prd_launcher import run_prd_wizard
    ...
    if "--init" in args:
        from ralph.prd_generator import generate_prd, generate_tasks
    ...
    if "--plan" in args:
        from ralph.prd_generator import generate_tasks
