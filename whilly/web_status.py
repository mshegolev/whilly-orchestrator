"""Lightweight HTTP status endpoint for Whilly orchestrator.

Provides ``/api/status`` (JSON) and ``/`` (HTML) on localhost:9191.
Uses only stdlib — no FastAPI/Flask dependencies.

Usage:
    from whilly.web_status import WebStatusServer
...
log = logging.getLogger("whilly.web")
...
class WebStatusServer:
    """HTTP server for Whilly status on localhost."""
