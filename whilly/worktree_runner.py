"""Git worktree isolation for parallel Whilly agents.
...
log = logging.getLogger("whilly.worktree")

_WORKTREE_BASE = ".whilly_worktrees"
_PLAN_WORKSPACE_BASE = ".whilly_workspaces"
...
class WorktreeManager:
    ...
    def create(self, task_id: str) -> Worktree:
        ...
        Creates branch ``whilly/{task_id}`` and worktree at
        ``{base_dir}/{task_id}``.
    ...
    def merge_back(self, task_id: str) -> MergeResult:
        ...
        subprocess.run(
            ["git", "commit", "-m", f"Whilly merge: {task_id} ({len(commits)} commits)"],
            capture_output=True,
            text=True,
            timeout=10,
        )
...
def create_plan_workspace(
    ...
    Args:
        ...
        base_dir: Корень для всех workspaces (``.whilly_workspaces``).
    ...
    """
    branch = f"whilly/workspace/{slug}"
