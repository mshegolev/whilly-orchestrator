...
class ResourceMonitor:
            ...
            if psutil:
                for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                    try:
                        ...
                        if ('whilly' in cmdline_str.lower() or
                            'claude' in cmdline_str.lower() and 'whilly' in cmdline_str):
                            count += 1
    ...
    def check_limits(self, usage: Optional[ResourceUsage] = None) -> Dict[str, Any]:
        ...
        if log_size > self.limits.max_log_dir_size_gb:
            violations['log_dir'] = {
                'current': log_size,
                'limit': self.limits.max_log_dir_size_gb,
                'severity': 'low'
            }
    ...
    def get_recommendation(self, violations: Dict[str, Any]) -> str:
        ...
        if 'cpu' in violations:
            recommendations.append(f"🔴 CPU usage too high ({violations['cpu']['current']:.1f}%). "
                                 f"Consider reducing WHILLY_MAX_PARALLEL or waiting for processes to complete.")

        if 'memory' in violations:
            recommendations.append(f"🔴 Memory usage too high ({violations['memory']['current']:.1f}%). "
                                 f"Close other applications or reduce concurrent processes.")

        if 'disk' in violations:
            recommendations.append(f"🔴 Low disk space ({violations['disk']['current']:.1f}GB free). "
                                 f"Free up disk space or change working directory.")

        if 'processes' in violations:
            recommendations.append(f"🔴 Too many processes ({violations['processes']['current']}). "
                                 f"Wait for some processes to complete.")

        if 'log_dir' in violations:
            recommendations.append(f"⚠️ Log directory large ({violations['log_dir']['current']:.1f}GB). "
                                 f"Consider cleaning up old logs.")
