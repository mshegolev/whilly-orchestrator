...
Usage::
    ...
    whilly worker launch jira-eord-9843        # binds to specific plan
...
def run_remove_command(argv: list[str]) -> int:
    ...
    * ``whilly worker remove jira-eord-9843`` — drop one (uses default
      control_url, errors out if ambiguous).
    * ``whilly worker remove jira-eord-9843 --connect http://...`` —
