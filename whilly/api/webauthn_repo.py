"""Async DB repository for the WebAuthn data layer — ``webauthn_credentials``
(migration 026) plus the opaque per-user ``webauthn_user_handles`` (migration 028).
...
async def get_or_create_user_handle(pool: asyncpg.Pool, *, username: str) -> bytes:
    """Return the stable opaque WebAuthn user handle for ``username``.

    Creates a random 32-byte handle on first call and returns the same handle
    thereafter (E15 Finding 3 — the spec wants an opaque, rename-stable handle as
    ``user.id``, not the PII username). The upsert is atomic: on a concurrent /
    repeat call the ``ON CONFLICT`` keeps the existing row and ``RETURNING`` hands
    back the already-stored handle, so the freshly-generated bytes are discarded.
    Multiple passkeys for one user therefore share one handle.
    """
    if not isinstance(username, str) or not username.strip():
        raise ValueError("get_or_create_user_handle: username must be a non-empty string")
    normalised = username.strip().lower()
    async with pool.acquire() as conn:
        handle = await conn.fetchval(
            """
            INSERT INTO webauthn_user_handles (username, user_handle)
            VALUES ($1, $2)
