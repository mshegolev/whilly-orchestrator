...
Whilly can resume from where it left off after an unexpected crash
(OOM, SIGKILL, etc.).
"""
...
class StateStore:
    ...
    def __init__(self, state_file: str = ".whilly_state.json"):
    ...
    def discover_tmux_sessions(self) -> list[dict]:
        """Find whilly-* tmux sessions that are still running."""
        try:
            r = subprocess.run(
        ...
        for line in r.stdout.strip().splitlines():
            if not line.startswith("whilly-"):
                continue
            parts = line.split(":", 1)
            session_name = parts[0]
            activity = int(parts[1]) if len(parts) > 1 and parts[1].isdigit() else 0
            task_id = session_name.removeprefix("whilly-")
            sessions.append(
                {
