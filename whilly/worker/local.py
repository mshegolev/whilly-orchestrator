...
actually runs tasks. The 019a slice is deliberately minimal — no heartbeat,
no signal handling, no CLI; those land in 019b1 / 019b2 / 019c respectively.
This file is just the loop body so the testable surface stays small enough
to exercise exhaustively without spinning up Postgres or the ``claude``
binary.
...
Termination
-----------
Production callers loop forever and rely on asyncio cancellation
(SIGTERM in TASK-019b2) to break out. Tests pass ``max_iterations=N`` so
each test runs deterministically in milliseconds.
...
async def run_local_worker(
    ...
    *,
    ...
    See module docstring for the full per-iteration contract. This
    function never returns "early" on its own — it loops until either
    ``max_iterations`` is reached (test-only) or an outer
    :class:`asyncio.CancelledError` is raised (production shutdown).
    ...
    repo:
        ...
        and :meth:`~whilly.adapters.db.repository.TaskRepository.fail_task`.
    runner:
