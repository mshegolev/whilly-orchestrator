"""Interactive PRD Wizard — inline PRD creation from running Ralph TUI.

Launches Claude CLI in conversational mode with the PRD-creator system prompt.
Runs in a background thread or tmux pane so Ralph's main loop continues.
...
log = logging.getLogger("ralph.prd_wizard")
...
class PrdWizard:
    """Interactive PRD creation wizard that runs alongside Ralph."""
    ...
    def _run_claude_interactive(self, idea: str, prd_path: Path) -> bool:
        ...
        session_name = "ralph-prd-wizard"

        # Write system prompt to temp file (Claude --system-prompt flag)
        prompt_file = Path("/tmp/ralph_prd_prompt.md")
        ...
        claude_cmd = (
            ...
            f'--system-prompt "$(cat /tmp/ralph_prd_prompt.md)" '
        ...
        self._status(
            ...
            "Переключись: tmux attach -t ralph-prd-wizard"
        )
