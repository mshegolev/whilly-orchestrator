...
| `glab` CLI | GitLab token fallback | [ASSUMED] — present in MTS dev env per CLAUDE.md but not verified at research time | — | `GITLAB_TOKEN` / `GITLAB_API_TOKEN` env var (no glab needed) |
