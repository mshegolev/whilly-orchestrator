"""Tests for whilly error handling (R2-017)."""

from whilly.agent_runner import AgentResult, is_api_error, is_auth_error
...
def test_notify_noop_when_disabled(monkeypatch):
    """Notifications should be noop when WHILLY_VOICE=0."""
    import importlib

    import whilly.notifications as notif

    monkeypatch.setenv("WHILLY_VOICE", "0")
    # Re-import to pick up env change
    importlib.reload(notif)
    # Should not crash even with say unavailable
    notif.notify("test")
    notif.notify_task_done()
    monkeypatch.delenv("WHILLY_VOICE")
    importlib.reload(notif)
