"""Tests for Whilly v3 features: F1 (Cost Budget Guard) and F2 (Deadlock Detection)."""
...
class TestBudgetConfig:
    ...
    def test_budget_from_env(self):
        with patch.dict(os.environ, {"WHILLY_BUDGET_USD": "25.5"}):
            config = WhillyConfig.from_env()
            assert config.BUDGET_USD == 25.5

    def test_budget_zero_unlimited(self):
        with patch.dict(os.environ, {"WHILLY_BUDGET_USD": "0"}):
            config = WhillyConfig.from_env()
...
class TestMaxTaskRetriesConfig:
    ...
    def test_max_retries_from_env(self):
        with patch.dict(os.environ, {"WHILLY_MAX_TASK_RETRIES": "8"}):
            config = WhillyConfig.from_env()
...
class TestBudgetNotifications:
    @patch("whilly.notifications.SAY_BIN", None)
