"""Tests for Ralph F4: Headless/CI Mode.
...
from ralph.dashboard import NullDashboard

# ── Load ralph.py as 'ralph_main' to avoid shadowing by ralph/ package ───


def _load_ralph_main():
    """Import ralph.py script as 'ralph_main' module."""
    ralph_py = Path(__file__).resolve().parent.parent / "ralph.py"
    spec = importlib.util.spec_from_file_location("ralph_main", str(ralph_py))
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


ralph_main = _load_ralph_main()
...
class TestHeadlessConfig:
    ...
    def test_headless_from_env(self):
        with patch.dict(os.environ, {"RALPH_HEADLESS": "1"}):
            config = RalphConfig.from_env()
            assert config.HEADLESS is True

    def test_headless_from_env_false(self):
        with patch.dict(os.environ, {"RALPH_HEADLESS": "0"}):
            config = RalphConfig.from_env()
            assert config.HEADLESS is False
    ...
    def test_timeout_from_env(self):
        with patch.dict(os.environ, {"RALPH_TIMEOUT": "3600"}):
            config = RalphConfig.from_env()
...
class TestExitCodes:
    def test_exit_code_constants(self):
        assert ralph_main.EXIT_SUCCESS == 0
        assert ralph_main.EXIT_SOME_FAILED == 1
        assert ralph_main.EXIT_BUDGET_EXCEEDED == 2
        assert ralph_main.EXIT_TIMEOUT == 3
...
class TestHeadlessOutput:
    def test_emit_json_writes_to_stdout(self, capsys):
        ralph_main._emit_json({"event": "progress", "done": 5, "total": 10})
...
class TestRunPlanHeadless:
    ...
    def test_headless_produces_json_output(self, done_plan_file, tmp_path, capsys):
        ...
        config = RalphConfig(HEADLESS=True, LOG_DIR=str(tmp_path / "logs"), MAX_PARALLEL=1)

        with (
            patch.object(ralph_main, "Reporter") as MockReporter,
            patch.object(ralph_main, "needs_decompose", return_value=False),
            patch.object(ralph_main, "notify_plan_done"),
        ):
            ...
            result = ralph_main.run_plan(str(done_plan_file), config, "test-agent")
    ...
    def test_exit_code_0_on_success(self, done_plan_file, tmp_path):
        ...
        assert result is not None
        _, exit_code = result
        assert exit_code == ralph_main.EXIT_SUCCESS
    ...
    def test_exit_code_1_on_failures(self, failed_plan_file, tmp_path):
        ...
        with (
            ...
            result = ralph_main.run_plan(str(failed_plan_file), config, "test-agent")

        assert result is not None
        _, exit_code = result
        assert exit_code == ralph_main.EXIT_SOME_FAILED
    ...
    def test_timeout_triggers_graceful_stop(self, plan_file, tmp_path):
        ...
        with (
            ...
            patch.object(ralph_main, "needs_decompose", return_value=False),
            patch.object(ralph_main, "notify_plan_done"),
            patch.object(ralph_main, "run_agent_async", return_value=mock_proc),
            patch.object(ralph_main, "collect_result") as mock_collect,
            patch.object(ralph_main, "kill_all_ralph_sessions"),
        ):
            mock_reporter = MagicMock()
            mock_reporter.totals = MagicMock()
            mock_reporter.totals.cost_usd = 0.0
            ...
            result = ralph_main.run_plan(str(plan_file), config, "test-agent")

        assert result is not None
        _, exit_code = result
        assert exit_code == ralph_main.EXIT_TIMEOUT
...
class TestAutoHeadless:
    ...
    def test_auto_headless_when_no_tty_in_main(self):
        ...
        with (
            ...
            patch.object(ralph_main, "RalphConfig") as MockConfig,
            patch.object(ralph_main, "StateStore"),
        ):
            mock_stdout.isatty.return_value = False
            config = RalphConfig()
