"""Tests for whilly.state_store — F3 Crash Resume with Persistent State."""
...
def tmp_state_file(tmp_path):
    ...
    return str(tmp_path / ".whilly_state.json")
...
class TestAtomicWrite:
    def test_atomic_write_uses_tmp_and_rename(self, tmp_state_file):
        ...
        tmp_files = list(parent.glob(".whilly_state_*.tmp"))
...
class TestDiscoverTmuxSessions:
    @patch("whilly.state_store.subprocess.run")
    def test_discover_finds_whilly_sessions(self, mock_run):
        """discover_tmux_sessions returns whilly-* sessions."""
        mock_run.return_value = MagicMock(
            returncode=0,
            stdout="whilly-TASK-001:1711929600\nwhilly-TASK-002:1711929700\nother-session:1711929800\n",
        )
        store = StateStore(".whilly_state.json")
        sessions = store.discover_tmux_sessions()

        assert len(sessions) == 2
        assert sessions[0]["task_id"] == "TASK-001"
        assert sessions[0]["session_name"] == "whilly-TASK-001"
