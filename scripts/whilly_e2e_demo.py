...
def main() -> int:
    ...
    if not refreshed:
        log(f"main agent: task {task.id} disappeared from plan — exiting")
        return 0
    if refreshed.status != "done":
        # Sequential whilly sometimes succeeds (agent emits COMPLETE + commits)
        # but doesn't flip status in tasks.json itself. Fall back to: if the
        # working branch has new commits relative to origin/main, treat the
        # task as done and proceed to the review loop.
        new_commits_proc = _git("rev-list", "--count", f"origin/main..{branch}", cwd=Path.cwd())
        try:
            new_commits = int((new_commits_proc.stdout or "0").strip())
        except ValueError:
            new_commits = 0
        if new_commits > 0:
            log(
                f"main agent: tasks.json still pending but branch has {new_commits} new commit(s) "
                f"vs origin/main — treating as done (agent path: COMPLETE-but-no-state-flip)"
            )
            tm.mark_status([task.id], "done")
            tm.reload()
            refreshed = tm.get_task(task.id)
            event("main.recovered", task_id=task.id, new_commits=new_commits)
        else:
            log(f"main agent did not mark {task.id} done (status={refreshed.status}) and no new commits — exiting")
            return 0
