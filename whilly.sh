...
if [[ "${1:-}" == "--all" ]]; then
    ...
    echo "Usage: whilly.sh [OPTIONS] [PLAN_FILE...]"
    echo ""
    echo "  whilly.sh                          Use tasks.json or interactive menu"
    echo "  whilly.sh plan1.json plan2.json    Run specific plan files"
    echo "  whilly.sh --all                    Run all discovered plans"
    echo "  whilly.sh -h, --help               Show this help"
    echo ""
    ...
    echo "  WHILLY_AGENT=NAME     Force agent (claude/claude-1m/claude-1m-4.7/codex)"
    exit 0
elif [[ $# -gt 0 ]]; then
    for f in "$@"; do
...
init_report() {
    ...
    REPORT_FILE=".planning/reports/whilly_$(basename "$plan_file" .json)_$(date +%Y%m%d_%H%M%S).json"
...
generate_summary_report() {
    ...
    local summary_file
    summary_file=".planning/reports/whilly_summary_$(date +%Y%m%d_%H%M%S).md"
...
show_dashboard() {
    ...
    local title
    title=" WHILLY  ◆  $(elapsed)  ◆  $(echo "$agent" | tr '[:lower:]' '[:upper:]')  ◆  iter ${iter}/${MAX_ITERATIONS}"
...
# Log viewer: last N lines of whilly.log
show_log_viewer() {
    local rows
    rows=$(tput lines 2>/dev/null || echo 40)
...
show_help() {
    local help_lines=(
        ...
        ""
        "  ${BGD}${WH} q ${R}  ${B}Quit${R} — gracefully stop agent and exit Whilly"
        "  ${BGD}${WH} d ${R}  ${B}Detail${R} — show full detail for a specific task (by ID)"
        "  ${BGD}${WH} l ${R}  ${B}Log${R} — show last lines of whilly.log (agent output)"
        ...
        "  WHILLY_AGENT=...       force agent (claude/claude-1m/claude-1m-4.7/codex)"
        ""
        "${B}Files:${R}"
        ""
        ...
        ""
        "${D}Tip: run 'tail -f whilly.log' in another terminal for full agent output${R}"
    )
        ...
        if [[ -n "$key" ]]; then
            case "$key" in
                q|Q)
                    ...
                    echo -e "  ${BGD}${WH} x ${R}  Exit Whilly completely"
                    echo ""
                    echo -ne " Choice: "
...
resolve_agent() {
    if [[ -n "${WHILLY_AGENT:-}" ]]; then echo "$WHILLY_AGENT"; return 0; fi
...
run_agent() {
    ...
    case "$agent" in
        ...
        codex)
            local of; of="$(mktemp -t whilly_codex.XXXXXX)"
...
run_agent_interactive() {
    ...
    result_file="$(mktemp -t whilly_result.XXXXXX)"
...
run_orchestrator() {
    ...
    local orch_result
    orch_result="$(mktemp -t whilly_orch.XXXXXX)"
...
run_interface_agreement() {
    ...
    local rfile
    rfile="$(mktemp -t whilly_iface.XXXXXX)"
...
run_parallel_batch() {
    ...
    for tid in "${task_ids[@]}"; do
        ...
        local rfile
        rfile="$(mktemp -t "whilly_${tid}.XXXXXX")"
...
cleanup() {
    ...
    if (( exit_code != 0 && exit_code != 130 )); then
        ...
        echo "[$(date '+%H:%M:%S')] $msg" >> "${LOG_FILE:-whilly_crash.log}"
        echo -e "\n${RD}${B}Whilly crashed:${R} $msg" >&2
        echo -e "${D}Log: ${LOG_FILE:-whilly_crash.log}${R}" >&2
    fi
}
on_error() {
    ...
    echo "[$(date '+%H:%M:%S')] ERROR exit=$exit_code line=$line cmd='$cmd'" >> "${LOG_FILE:-whilly_crash.log}"
}
trap on_error ERR
trap cleanup EXIT
...
agent=$(resolve_agent) || {
    echo -e "${RD}No agent found. Install claude or codex, or set WHILLY_AGENT.${R}" >&2
    exit 1
}
...
for TASKS_FILE in "${PLAN_FILES[@]}"; do
    ((PLAN_INDEX++)) || true
    LOG_FILE="whilly_$(basename "$TASKS_FILE" .json).log"
    START_TIME=$(date +%s)
    iteration=1
    INITIAL_TASK_COUNT=0
    LAST_TASK_COUNT=0
    ...
    if (( PLAN_TOTAL > 1 )); then
        ...
        log_event "Whilly started: $TASKS_FILE (max=$MAX_ITERATIONS)"
    fi
...
Если при реализации фичи ты заметишь, что TASK полностью выполнен, выведи <promise>COMPLETE</promise>."
        ...
        if (( agent_exit != 0 )); then
            ...
            show_dashboard "$agent" "$iteration" \
                "${RD}${B}✗ Agent error (exit $agent_exit)${R} ${D}— see whilly.log${R}"
            rm -f "$result_file"
            ((iteration++))
            sleep 3
            continue
