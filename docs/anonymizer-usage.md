...
The standalone package is the Acme↔Acme redaction proxy as a general-purpose
...
- ✅ Case-sensitive company name replacement (acme, Acme, Acme → Acme)
...
```bash
$ claude -p "I work at acme, what should I do?"
```
→ Sends to API: "I work at acme, what should I do?"
→ Logs show: "I work at acme, what should I do?"

**With anonymizer enabled:**
```bash
$ WHILLY_ENABLE_ANONYMIZER=1 claude -p "I work at acme, what should I do?"
```
→ Sends to API: "I work at Acme, what should I do?"
→ Logs show: "Sent anonymized prompt: I work at Acme..."
→ Returns to user: "...response mentioning acme..."
...
1. **`anonymizer.py`** – Core anonymization logic
   ...
   - Tracks which specific original value was used (e.g., "acme" vs "Acme")
...
async def main():
    ...
    print(result.output)  # Output: "I work at acme" (deanonymized)
...
- Replacement is case-sensitive (acme ≠ Acme for matching purposes)
