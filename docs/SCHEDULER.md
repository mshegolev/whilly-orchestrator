...
issues = execute_jql("project = ACME AND status = Open", max_results=50)
```
...
if event.matches_rule("project = ACME AND status = Open"):
    # Process event
```
...
✓ Configuration valid (2 rules)
  - rule-1: ACME Open Bugs [enabled]
  - rule-2: ACME Features [disabled]
```
...
jql_filter = "project = ACME AND type = Bug AND status = Open"
...
[[rules]]
id = "acme-features"
name = "ACME Feature Requests"
jira_project_key = "ACME"
jql_filter = "project = ACME AND type = Feature AND status = Backlog"
