...
|---|---|---|
| Компетенция Whilly-loop (Ralph-loop + TRIZ/Decision Gate) / agent orchestration | каждый участник | в процессе |
...
- **Ralph Wiggum loop** — исходная техника непрерывного цикла, где AI-агент берёт задачу за задачей. Названа по фразе "I'm helping!" из Симпсонов. См. [Ghuntley's post](https://ghuntley.com/ralph/).
- **Whilly Wiggum loop** — «умный брат» Ralph-loop'а. Тот же принцип «взял → попробовал → повторил», плюс TRIZ-анализ противоречий, Decision Gate (отказ от заведомо мусорных задач до траты токенов) и PRD wizard (понять проблему перед тем как кодить). Whilly — фамильный ответ сообщества на «вроде работает, но мозгов мало» у ванильного Ralph-loop'а.
